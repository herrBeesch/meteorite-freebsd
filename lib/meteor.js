...
Meteor = function(config) {
  ...
  console.log(config);
  config.git = 'https://github.com/herrBeesch/meteor-freebsd.git';
...
Meteor.prototype.install = function(fn) {
  ...
  if (!fs.existsSync(dev_bundle)) {
    // running the custom generate-dev-bundle.sh script
    console.log('Downloading Meteor development bundle');
    var meteor = spawn('sh', ['generate-dev-bundle.sh'], {cwd: self.source.path + '/admin/'});
...
Meteor.prototype.execute = function(args, package_dir, fn) {
  ...
  self.ensurePrepared(function() {
    Command.execute(path.join(self.source.packagePath(), 'meteor-freebsd'), args, package_dir, fn);
  });
};
